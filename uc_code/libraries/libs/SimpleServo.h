 ...
 * 	4.- The sense value limits the max angle value, to avoid extremes in bad servos
 *
 */
...
public:
	SimpleServoClass(int8_t sense);							// Associates the class with a Servo attach pin
	SimpleServoClass(int min, int max, int8_t sense);		// Associates the class with a Servo attach pin and sets min and max values in uS
	...
	void attach(uint8_t servoPin);							// Attach needs to be done in runtime
private:
